...
LABEL_DESCRIPTIONS = {
    "pillar:security": "WAF Security pillar finding",
    "pillar:reliability": "WAF Reliability pillar finding",
    "pillar:cost": "WAF Cost Optimization pillar finding",
    "pillar:operations": "WAF Operational Excellence pillar finding",
    "pillar:performance": "WAF Performance pillar finding",
    "pillar:governance": "Governance & landing zone finding",
    "severity:critical": "Must fix — security or reliability risk",
    "severity:high": "Should fix soon — significant impact",
    "severity:medium": "Plan to fix — moderate impact",
    "severity:low": "Nice to have — low impact improvement",
    "assessment-finding": "Auto-generated by Azure Environment Advisor",
}
...
def create_github_issue(finding: Finding, labels: list[str], milestone: str = "", dry_run: bool = False) -> bool:
...
def main():
    ...
    parser.add_argument("--milestone", default="", help="Group issues under a milestone (e.g., 'Assessment 2026-03-31'). Created automatically if it doesn't exist.")
    ...
    if milestone and not args.dry_run:
        subprocess.run(
            ["gh", "api", "repos/{owner}/{repo}/milestones", "-f", f"title={milestone}"],
            capture_output=True, text=True, timeout=15,
        )
    ...
    for finding in filtered:
        success = create_github_issue(finding, extra_labels, milestone=milestone, dry_run=args.dry_run)
        if success:
            created += 1
        else:
            failed += 1
